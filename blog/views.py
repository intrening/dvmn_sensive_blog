...
def index(request):
    ...
    sorted_popular_posts = Post.objects.annotate(Count('likes')).order_by('-likes__count')
    most_popular_posts = list(sorted_popular_posts)[0:5]
    ...
    most_fresh_posts = list(fresh_posts)[0:5]

    sorted_popular_tags = Tag.objects.annotate(Count('posts')).order_by('-posts__count')
    most_popular_tags = sorted_popular_tags[0:5]
